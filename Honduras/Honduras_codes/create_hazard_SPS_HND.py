...
# create meshgrid
[X, Y ] = np.meshgrid(np.linspace(lon_min,lon_max,884), np.linspace(lat_min,lat_max,884))
n_cen = lon.size # number of centroids
n_ev = 3 # number of events
...
def ev_name(string):
    ...
    if num_digits > 0:
      ...
      for digits in product("123", repeat=num_digits):  
...
#haz.event_name = ev_name('HND%d')
haz.event_name = ['fl_tr25_today_30m', 'fl_tr50_today_30m', 'fl_tr100_today_30m']
